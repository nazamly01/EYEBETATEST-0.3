function renderContactPage(t) {
  ...
  if (wa) {
    cards.push(
      `<div class="contact-card"><span class="contact-card-label">WhatsApp</span><a class="contact-card-value" href="${waHref}" target="_blank" rel="noopener noreferrer">${escapeHtml(
        t.whatsappLabel || 'Message us'
      )}</a></div>`
    );
  }
  if (address) {
      ...
      ${
        cards.length
          ? `<div class="contact-cards">${cards.join('')}</div>
             ${
               wa
                 ? `<a class="contact-wa-quick" href="${waHref}" target="_blank" rel="noopener noreferrer">${escapeHtml(
                     t.whatsappQuickLabel || 'Message us on WhatsApp'
                   )}</a>`
                 : ''
             }`

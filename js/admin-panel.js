...
function openOrderDetailModal(orderId) {
  ...
  wrap.innerHTML = `<div class="modal modal-order-detail" onclick="event.stopPropagation()">
    ...
    <div class="order-detail-body">
      ...
      ${o.payment_proof_url ? `<div class="od-label" style="margin:20px 0 10px">Payment Proof</div><div><a href="${escapeHtml(o.payment_proof_url)}" target="_blank" class="btn btn-outline" style="padding:6px 12px;font-size:10px">View Screenshot</a></div>` : ''}
  ...
  document.getElementById('productsTable').innerHTML = `
    ...
    <tbody>${products
      .map(
        (p) => `
      <tr>
        <td><img src="${escapeHtml(p.image || p.images?.[0])}" class="product-thumb" / loading="lazy"></td>
        ...
        <td>${(p.category || '').split(',').filter(Boolean).map(c => `<span class="badge badge-gray" style="margin-right:4px;margin-bottom:4px">${escapeHtml(c.trim())}</span>`).join('')}</td>
...
function updateImagePreviews() {
  ...
  preview.innerHTML = productImages
    ...
    <div class="img-preview-item">
      <img src="${escapeHtml(url)}" onerror="this.style.opacity=.3" / loading="lazy">
...
async function saveProduct() {
  ...
  let totalStock = 0;
  Object.values(sizeStocks).forEach(q => totalStock += q);
  ...
  const data = {
    ...
    category: Array.from(document.querySelectorAll('input[name="pCat"]:checked')).map(cb => cb.value).join(', '),
...
async function renderCouponsTable() {
  ...
  document.getElementById('couponsTable').innerHTML = `
    <thead><tr><th>Code</th><th>Type</th><th>Value</th><th>Expiry</th><th>Usage</th><th>Product</th><th>Actions</th></tr></thead>
    <tbody>${coupons
      .map(
        (c) => `
      <tr>
        ...
        <td style="font-size:11px;color:#aaa">${c.product_id ? 'Required' : 'Any'}</td>
...
async function openCouponModal() {
  ...
  document.getElementById('modalBody').innerHTML = `
    <div class="admin-form">
      ...
      <div class="form-field full"><label class="form-label">Required Product</label>
        <select class="form-select" id="cProd">
          <option value="">-- Any Product (Global) --</option>
...
async function saveCoupon() {
  const row = {
    ...
    product_id: document.getElementById('cProd').value || null,
  };
...
function printOrder(orderId) {
  ...
  if (rawAddr.includes('|')) {
    ...
    parts.forEach(p => {
      if (p.startsWith('Username:')) parsedName = p.replace('Username:', '').trim();
      if (p.startsWith('Phone:')) parsedPhone = p.replace('Phone:', '').trim();
    });
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; padding: 20mm; background-color: #050505; color: #fff; width: 100%; min-height: 297mm; box-sizing: border-box;">
      <div style="display: flex; justify-content: space-between; border-bottom: 2px solid #c9a84c; padding-bottom: 20px; margin-bottom: 30px;">
        <div>
          <h1 style="margin: 0; font-family: Georgia, serif; font-size: 32px; letter-spacing: 2px; text-transform: uppercase; color: #fff;">EYE</h1>
          <div style="font-size: 10px; letter-spacing: 2px; color: #c9a84c; text-transform: uppercase; margin-top: 4px;">Store</div>
        </div>
        <div style="text-align: right;">
          <h2 style="margin: 0; font-size: 24px; color: #c9a84c;">INVOICE</h2>
          <p style="margin: 4px 0 0; color: #ccc; font-size: 14px;">Order #${escapeHtml(o.id)}</p>
          <p style="margin: 4px 0 0; color: #ccc; font-size: 12px;">${d}</p>
        </div>
      </div>
      
      <div style="display: flex; justify-content: space-between; margin-bottom: 40px;">
        <div style="flex: 1; padding-right: 20px;">
          <h3 style="font-size: 12px; text-transform: uppercase; color: #c9a84c; border-bottom: 1px solid #333; padding-bottom: 8px; margin-bottom: 12px; letter-spacing: 1px;">Customer Details</h3>
          <p style="margin: 0 0 6px; font-weight: bold; font-size: 16px; color: #fff;">${cName}</p>
          <p style="margin: 0 0 6px; font-size: 14px; color: #ddd;"><strong>Phone:</strong> ${cPhone}</p>
          <p style="margin: 0; font-size: 14px; line-height: 1.5; color: #ddd;"><strong>Address:</strong><br>${cAddress.replace(/\n/g, '<br>')}</p>
        </div>
        <div style="flex: 1; padding-left: 20px;">
          <h3 style="font-size: 12px; text-transform: uppercase; color: #c9a84c; border-bottom: 1px solid #333; padding-bottom: 8px; margin-bottom: 12px; letter-spacing: 1px;">Order Status</h3>
          <p style="margin: 0 0 6px; font-size: 14px; color: #ddd;"><strong>Status:</strong> ${escapeHtml(o.status)}</p>
          <p style="margin: 0 0 6px; font-size: 14px; color: #ddd;"><strong>Payment Method:</strong> ${escapeHtml(o.payment_method)}</p>
          <p style="margin: 0; font-size: 14px; color: #ddd;"><strong>Payment Status:</strong> ${escapeHtml(o.payment_status)}</p>
        </div>
      </div>
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
        <thead>
          <tr>
            <th style="padding: 12px; text-align: left; background: #111; border-bottom: 2px solid #333; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; color: #c9a84c;">Item</th>
            <th style="padding: 12px; text-align: center; background: #111; border-bottom: 2px solid #333; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; color: #c9a84c;">Size</th>
            <th style="padding: 12px; text-align: center; background: #111; border-bottom: 2px solid #333; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; color: #c9a84c;">Qty</th>
            <th style="padding: 12px; text-align: right; background: #111; border-bottom: 2px solid #333; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; color: #c9a84c;">Price</th>
          </tr>
        </thead>
        <tbody>
          ${o.items.map(it => `
            <tr>
              <td style="padding: 12px; border-bottom: 1px solid #222; font-size: 14px; color: #fff;">${escapeHtml(it.name)}</td>
              <td style="padding: 12px; border-bottom: 1px solid #222; text-align: center; font-size: 14px; color: #ddd;">${escapeHtml(it.size)}</td>
              <td style="padding: 12px; border-bottom: 1px solid #222; text-align: center; font-size: 14px; color: #ddd;">${it.qty}</td>
              <td style="padding: 12px; border-bottom: 1px solid #222; text-align: right; font-size: 14px; color: #fff;">${formatPrice(it.price)}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <div style="display: flex; justify-content: flex-end;">
        <div style="width: 300px;">
          <div style="display: flex; justify-content: space-between; padding: 8px 0; font-size: 14px; color: #ddd;">
            <span>Subtotal</span><span>${formatPrice(Number(o.subtotal != null ? o.subtotal : o.total) || 0)}</span>
          </div>
          ${Number(o.discount) > 0 ? `<div style="display: flex; justify-content: space-between; padding: 8px 0; font-size: 14px; color: #8fd9a8;"><span>Discount</span><span>−${formatPrice(Number(o.discount))}</span></div>` : ''}
          <div style="display: flex; justify-content: space-between; padding: 8px 0; font-size: 14px; color: #ddd; border-bottom: 1px solid #333; margin-bottom: 8px; padding-bottom: 12px;">
            <span>Shipping</span><span>${Number(o.shipping) === 0 ? 'Free' : formatPrice(Number(o.shipping || 0))}</span>
          </div>
          <div style="display: flex; justify-content: space-between; padding: 8px 0; font-size: 18px; font-weight: bold; color: #c9a84c;">
            <span>Total</span><span>${formatPrice(o.total)}</span>
          </div>
        </div>
      </div>
      
      <div style="margin-top: 60px; text-align: center; font-size: 12px; color: #666; border-top: 1px solid #222; padding-top: 20px;">
        Thank you for shopping with EYE Store.
      </div>
    </div>
  `;
    ...
    const html = `
        <div style="font-family: Arial, sans-serif; padding: 20mm; background-color: #050505; color: #fff; width: 100%; min-height: 297mm; box-sizing: border-box;">
            <div style="text-align: center; border-bottom: 2px solid #c9a84c; padding-bottom: 20px; margin-bottom: 30px;">
                ...
                <div style="font-size: 14px; color: #c9a84c; margin-top: 8px; text-transform: uppercase; letter-spacing: 1px;">Executive Summary Report</div>
                <div style="font-size: 12px; color: #999; margin-top: 4px;">Generated on ${d}</div>
            </div>
            
            <div style="margin-bottom: 40px;">
                <h2 style="font-size: 18px; color: #c9a84c; border-bottom: 1px solid #333; padding-bottom: 8px; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px;">Live & Overview Stats</h2>
                <div style="display: flex; gap: 20px;">
                    <div style="flex: 1; border: 1px solid #333; background: #111; padding: 20px; border-radius: 4px; text-align: center;">
                        <div style="font-size: 12px; color: #999; text-transform: uppercase; letter-spacing: 1px;">All-Time Revenue</div>
                        <div style="font-size: 24px; font-weight: bold; color: #fff; margin-top: 10px;">${revAll}</div>
                    </div>
                    <div style="flex: 1; border: 1px solid #333; background: #111; padding: 20px; border-radius: 4px; text-align: center;">
                        <div style="font-size: 12px; color: #999; text-transform: uppercase; letter-spacing: 1px;">Orders (30 Days)</div>
                        <div style="font-size: 24px; font-weight: bold; color: #fff; margin-top: 10px;">${orders30}</div>
                    </div>
                    <div style="flex: 1; border: 1px solid #c9a84c; padding: 20px; border-radius: 4px; text-align: center; background: rgba(201,168,76,0.1);">
                        <div style="font-size: 12px; color: #c9a84c; text-transform: uppercase; letter-spacing: 1px; font-weight: bold;">Live Visitors</div>
                        <div style="font-size: 32px; font-weight: bold; color: #c9a84c; margin-top: 4px;">${liveVis}</div>
                    </div>
                </div>
                
                <div style="margin-top: 20px; border: 1px solid #c9a84c; padding: 20px; border-radius: 4px; background: rgba(201,168,76,0.1); text-align: center;">
                    <div style="font-size: 12px; color: #c9a84c; text-transform: uppercase; letter-spacing: 1px; font-weight: bold; margin-bottom: 10px;">Top Trending Product Now</div>
                    <div style="font-size: 20px; font-family: Georgia, serif; color: #fff;">${trendProd}</div>
                    <div style="font-size: 13px; color: #ccc; margin-top: 6px;">${trendStats}</div>
                </div>
            </div>
            
            <div style="margin-bottom: 40px; page-break-inside: avoid;">
                <h2 style="font-size: 18px; color: #c9a84c; border-bottom: 1px solid #333; padding-bottom: 8px; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px;">Top Products by Revenue</h2>
                <div style="border: 1px solid #333; border-radius: 4px; padding: 1px; background: #111;">
                    <style>
                        table { width: 100%; border-collapse: collapse; }
                        th { background: #1a1a1a !important; padding: 12px; text-align: left; font-size: 12px; color: #c9a84c !important; border-bottom: 1px solid #333 !important; text-transform: uppercase; letter-spacing: 1px; }
                        td { padding: 12px; border-bottom: 1px solid #222 !important; font-size: 14px; color: #fff !important; }
            ...
            <div style="margin-bottom: 40px; page-break-inside: avoid;">
                <h2 style="font-size: 18px; color: #c9a84c; border-bottom: 1px solid #333; padding-bottom: 8px; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px;">Detailed Product Engagement</h2>
                <div style="border: 1px solid #333; border-radius: 4px; padding: 1px; background: #111;">
                    ${viewsHTML}
                </div>
            </div>
            ...
            <div style="page-break-before: always; padding-top: 20px;">
                <h2 style="font-size: 18px; color: #c9a84c; border-bottom: 1px solid #333; padding-bottom: 8px; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px;">Recent Orders (Last 20)</h2>
                <div style="border: 1px solid #333; border-radius: 4px; padding: 1px; background: #111;">
                    <table>
                        <thead><tr><th>Order ID</th><th>Date</th><th>Total</th><th>Status</th></tr></thead>
                        <tbody>
                            ${_adminOrdersCache.slice(0, 20).map(o => `
                                <tr>
                                    <td style="font-family: monospace; color: #ccc;">${escapeHtml(o.id)}</td>
                                    <td style="color: #ccc;">${formatDate(o.date || o.created_at)}</td>
                                    <td style="font-weight: bold; color: #c9a84c;">${formatPrice(o.total)}</td>
                                    <td style="color: #ccc;">${escapeHtml(o.status)}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
...
async function renderLogsTable(filter = 'all') {
  ...
  document.getElementById('logsTable').innerHTML = `
    ...
    <tbody>${logs.map(l => {
    const actorName = l.profiles?.name || l.actor_name || l.actor_id || 'System';
